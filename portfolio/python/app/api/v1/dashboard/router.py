...
async def get_portfolio_dashboard_overview(
    ...
    try:
        overview = dashboard_service.get_dashboard_overview(portfolio_id, current_user.id)
        return overview
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e)) from e


@router.get("/portfolios/{portfolio_id}/performance-chart")
async def get_portfolio_performance_chart(
    ...
    try:
        chart_data = dashboard_service.get_portfolio_performance_chart_data(portfolio_id, days)
        return chart_data
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e)) from e


@router.get("/assets/{asset_id}/performance-chart")
