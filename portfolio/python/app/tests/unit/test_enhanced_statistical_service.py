...
def sample_request(sample_indicators):
    ...
    return IndicatorCalculationRequest(
        symbol="AAPL", period="6mo", interval="1d", indicators=sample_indicators
    )
...
class TestEnhancedStatisticalService:
    ...
    async def test_get_available_indicators(self, service):
        ...
        with patch.object(
            service.indicator_registry, "get_available_indicators"
        ) as mock_get:
            # Mock indicator objects with .dict() method
            mock_indicator = Mock()
            ...
            # Mock categories and statistics
            with patch.object(
                service.indicator_registry, "get_indicator_categories"
            ) as mock_categories:
                ...
                with patch.object(
                    service.indicator_registry, "get_indicator_statistics"
                ) as mock_stats:
                    mock_stats.return_value = {"total_indicators": 16}
    ...
    async def test_calculate_indicators_success(self, service, sample_request):
        ...
        mock_market_data.empty = False
        mock_market_data.columns = ["Date", "Open", "High", "Low", "Close", "Volume"]

        # Mock the Date column access
        mock_date_series = Mock()
        mock_date_series.min.return_value = datetime.now()
        mock_date_series.max.return_value = datetime.now()
        mock_market_data.__getitem__ = Mock(return_value=mock_date_series)

        with patch.object(
            service.market_data_service, "fetch_ticker_data", new_callable=AsyncMock
        ) as mock_get_data:
            mock_get_data.return_value = mock_market_data

            # Mock indicator registry
            with patch.object(
                service.indicator_registry, "validate_indicator_configuration"
            ) as mock_validate:
                mock_validate.return_value = []

                with patch.object(
                    service.indicator_registry, "calculate_indicators"
                ) as mock_calculate:
                    mock_result_df = Mock()
                    mock_result_df.to_dicts.return_value = [
                        {"Date": "2023-01-01", "Close": 150.0, "RSI": 50.0, "MACD": 0.1}
                    ]
                    mock_result_df.columns = ["Date", "Close", "RSI", "MACD"]
                    mock_result_df.__len__ = Mock(return_value=1)

                    # Mock the Date column access for metadata
                    mock_date_series = Mock()
                    mock_date_series.min.return_value = datetime.now()
                    mock_date_series.max.return_value = datetime.now()
                    mock_result_df.__getitem__ = Mock(return_value=mock_date_series)

                    # Mock performance metrics
                    mock_performance_metrics = Mock()
                    mock_performance_metrics.calculation_time_ms = 100.0
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    # Mock the date column access for chart metadata
                    mock_date_series = Mock()
                    mock_date_series.min.return_value = datetime.now()
                    mock_date_series.max.return_value = datetime.now()
                    mock_result_df.__getitem__ = Mock(return_value=mock_date_series)
    ...
    async def test_get_predefined_templates(self, service):
        ...
        with patch.object(
            service.configuration_service, "get_predefined_templates"
        ) as mock_get:
            # Mock template objects with .dict() method
            mock_template = Mock()
            ...
            mock_templates = {"basic_technical_analysis": mock_template}
            ...
            assert (
                result["basic_technical_analysis"]["name"] == "Basic Technical Analysis"
            )
    ...
    async def test_create_configuration_from_template(self, service):
        ...
        with patch.object(
            service.configuration_service, "create_from_template"
        ) as mock_create:
            mock_config = Mock()
    ...
    def test_prepare_chart_data_candlestick(self, service):
        ...
        result = service._prepare_chart_data(
            mock_df, indicators, "candlestick", True, {}
        )
    ...
    def test_prepare_chart_data_line(self, service):
        ...
        # Mock DataFrame
        mock_df = Mock()
        mock_df.to_dicts.return_value = [{"date": datetime(2023, 1, 1), "close": 152.0}]
    ...
    def test_prepare_indicator_data(self, service):
        ...
        # Mock DataFrame
        mock_df = Mock()
        mock_df.to_dicts.return_value = [{"date": datetime(2023, 1, 1), "rsi": 50.0}]
        mock_df.columns = ["date", "rsi"]
        ...
        with patch.object(
            service.indicator_registry, "get_indicator_definition"
        ) as mock_get_def:
            mock_def = Mock()
    ...
    def test_prepare_volume_data(self, service):
        """Test preparing volume data for indicator calculation response."""
        # Mock DataFrame
        mock_df = Mock()
        mock_df.to_dicts.return_value = [
            {"date": datetime(2023, 1, 1), "volume": 1000000}
        ]
