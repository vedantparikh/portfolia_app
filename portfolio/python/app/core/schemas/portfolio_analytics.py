...
class PortfolioPerformanceHistoryBase(BaseModel):
    ...
    total_unrealized_pnl_percent: Decimal = Field(..., description="Total unrealized P&L percentage")
...
class AssetPerformanceMetricsBase(BaseModel):
    ...
    # Volume indicators
    volume_sma: Optional[Decimal] = Field(None, description="Volume Simple Moving Average")
    ...
    total_return_6m: Optional[Decimal] = Field(None, description="6-month total return")
    total_return_1y: Optional[Decimal] = Field(None, description="1-year total return")
    total_return_3y: Optional[Decimal] = Field(None, description="3-year total return")
    total_return_5y: Optional[Decimal] = Field(None, description="5-year total return")
...
class PortfolioAllocationBase(BaseModel):
    portfolio_id: int
    asset_id: int
    target_percentage: Decimal = Field(..., ge=0, le=100, description="Target allocation percentage")
    min_percentage: Optional[Decimal] = Field(None, ge=0, le=100, description="Minimum allocation percentage")
    max_percentage: Optional[Decimal] = Field(None, ge=0, le=100, description="Maximum allocation percentage")
    rebalance_threshold: Optional[Decimal] = Field(None, ge=0, le=100, description="Rebalance trigger threshold")
...
class RebalancingEventBase(BaseModel):
    portfolio_id: int
    event_date: datetime
    event_type: str = Field(..., description="Event type: scheduled, threshold_triggered, manual")
    trigger_reason: Optional[str] = Field(None, description="Reason for rebalancing")
    pre_rebalance_value: Decimal = Field(..., description="Portfolio value before rebalancing")
    pre_rebalance_allocations: Optional[str] = Field(None, description="JSON of pre-rebalance allocations")
    rebalancing_actions: Optional[str] = Field(None, description="JSON of rebalancing actions")
    post_rebalance_value: Optional[Decimal] = Field(None, description="Portfolio value after rebalancing")
    post_rebalance_allocations: Optional[str] = Field(None, description="JSON of post-rebalance allocations")
    ...
    status: str = Field(default="pending", description="Event status: pending, completed, failed")
...
class PortfolioRiskMetricsBase(BaseModel):
    ...
    risk_score: Optional[Decimal] = Field(None, ge=0, le=100, description="Risk score 0-100")
    
    # Diversification metrics
    concentration_risk: Optional[Decimal] = Field(None, description="Concentration risk (Herfindahl index)")
    effective_number_of_assets: Optional[Decimal] = Field(None, description="Effective number of assets")
    diversification_ratio: Optional[Decimal] = Field(None, description="Diversification ratio")
    
    # Correlation metrics
    average_correlation: Optional[Decimal] = Field(None, description="Average correlation")
...
class AssetCorrelationBase(BaseModel):
    ...
    is_significant: Optional[bool] = Field(None, description="Whether correlation is statistically significant")
...
class PortfolioBenchmarkBase(BaseModel):
    ...
    benchmark_type: str = Field(..., description="Benchmark type: index, custom, peer_group")
    tracking_error: Optional[Decimal] = Field(None, description="Tracking error")
    information_ratio: Optional[Decimal] = Field(None, description="Information ratio")
    ...
    is_primary: bool = Field(default=False, description="Whether this is the primary benchmark")
...
class PortfolioAllocationAnalysis(BaseModel):
    ...
    # Current allocations
    current_allocations: List[dict] = Field(..., description="Current asset allocations")
    ...
    assets_requiring_rebalance: List[dict] = Field(..., description="Assets requiring rebalancing")
    
    # Rebalancing recommendations
    rebalancing_actions: List[dict] = Field(..., description="Recommended rebalancing actions")
    estimated_cost: Optional[Decimal] = Field(None, description="Estimated rebalancing cost")
...
class PortfolioPerformanceAnalysis(BaseModel):
    ...
    asset_contributions: List[dict] = Field(..., description="Asset contribution to performance")
    sector_contributions: List[dict] = Field(..., description="Sector contribution to performance")
    
    # Performance trends
    performance_trend: str = Field(..., description="Performance trend: improving, declining, stable")
...
class PortfolioRebalancingRecommendation(BaseModel):
    ...
    # Rebalancing trigger
    trigger_reason: str = Field(..., description="Reason for rebalancing recommendation")
    current_drift: Decimal = Field(..., description="Current allocation drift")
    
    # Recommended actions
    rebalancing_actions: List[dict] = Field(..., description="Recommended buy/sell actions")
    estimated_cost: Optional[Decimal] = Field(None, description="Estimated transaction costs")
    tax_impact: Optional[Decimal] = Field(None, description="Estimated tax impact")
    
    # Expected outcomes
    expected_allocations: List[dict] = Field(..., description="Expected post-rebalancing allocations")
    expected_risk_reduction: Optional[Decimal] = Field(None, description="Expected risk reduction")
    expected_return_impact: Optional[Decimal] = Field(None, description="Expected return impact")
