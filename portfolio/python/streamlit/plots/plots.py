...
class MacdRsiVolumeCandelstickChart:
    def get_closed_dates(self, df):
        ...
        closed_dates = [
            day for day in timeline.strftime("%Y-%m-%d").tolist() if day not in df_dates
        ]
    ...
    def plot_candlestick_chart(
        self, fig, df, row, column=1, plot_EMAs=True, plot_strategy=True
    ):
    ...
    def plot_RSI(self, fig, df, row, column=1):
        ...
        for y_pos, text, color in zip(
            [64, 36], ["Overvalued", "Undervalued"], ["Red", "Green"]
        ):
            fig.add_annotation(
                x=df["Date"].iloc[int(df["Date"].shape[0] / 10)],
                y=y_pos,
                text=text,
    ...
    def plot_volume(self, fig, df, row, column=1):
        ...
        fig.add_trace(
            go.Bar(
                ...
                y=df["Volume"],
                marker=dict(
                    color="lightskyblue", line=dict(color="firebrick", width=0.1)
                ),
                showlegend=False,
                name="Volume",
            ),
    ...
    def plot(self, df, ticker):
        ...
        fig = self.plot_candlestick_chart(
            fig, df, row=1, plot_EMAs=True, plot_strategy=True
        )
        ...
        fig.update_xaxes(
            rangebreaks=[dict(values=closed_dates_list)],
            range=[
                df["Date"].iloc[0] - datetime.timedelta(days=3),
        ...
        fig.update_layout(
            xaxis1=dict(
                linewidth=axis_lw, linecolor=axis_color, mirror=True, showgrid=False
            ),
            yaxis1=dict(
                linewidth=axis_lw, linecolor=axis_color, mirror=True, showgrid=False
            ),
