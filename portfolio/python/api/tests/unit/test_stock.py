...
with patch('yahooquery.search') as mock_search:
    with patch('services.market_data_service.market_data_service') as mock_market_service:
        with patch('app.core.auth.dependencies.get_optional_current_user') as mock_get_user:
...
class TestStockRouter:
    ...
    def test_get_symbols_success_authenticated(self, client, sample_quotes_data):
        ...
        with patch('api.v1.market.stock.search', return_value=sample_quotes_data):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
    ...
    def test_get_symbols_no_results(self, client):
        """Test symbol search with no results."""
        # Setup mocks
        with patch('api.v1.market.stock.search', return_value={"quotes": []}):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbols?name=INVALID")

                # Verify 404 response
                assert response.status_code == 404
                assert "No symbols found" in response.json()["detail"]

    def test_get_symbols_malformed_response(self, client):
        """Test symbol search with malformed response."""
        # Setup mocks
        with patch('api.v1.market.stock.search', return_value={"quotes": "invalid"}):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbols?name=AAPL")
    ...
    def test_get_symbols_exception(self, client):
        """Test symbol search with exception."""
        # Setup mocks
        with patch('api.v1.market.stock.search', side_effect=Exception("API Error")):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbols?name=AAPL")
                ...
                # Verify 500 response
                assert response.status_code == 500
                assert "Error searching for symbols" in response.json()["detail"]

    # Test /symbol-data/fresh endpoint
    def test_get_symbol_data_fresh_success_authenticated(self, client, sample_market_data):
        """Test successful fresh data retrieval for authenticated user."""
        # Setup mocks
        with patch('api.v1.market.stock.market_data_service.fetch_ticker_data', return_value=sample_market_data):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbol-data/fresh?name=AAPL&period=1d&interval=1m")
    ...
    def test_get_symbol_data_fresh_no_data(self, client):
        """Test fresh data endpoint with no data available."""
        # Setup mocks
        with patch('api.v1.market.stock.market_data_service.fetch_ticker_data', return_value=None):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbol-data/fresh?name=INVALID")

                # Verify 404 response
                assert response.status_code == 404
                assert "No fresh data available" in response.json()["detail"]

    def test_get_symbol_data_fresh_exception(self, client):
        """Test fresh data endpoint with exception."""
        # Setup mocks
        with patch('api.v1.market.stock.market_data_service.fetch_ticker_data', side_effect=Exception("Service Error")):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbol-data/fresh?name=AAPL")
    ...
    def test_get_symbol_data_local_success_authenticated(self, client, sample_market_data):
        ...
        mock_get_data = MagicMock(return_value=sample_market_data)
        with patch('api.v1.market.stock.market_data_service.get_market_data', mock_get_data):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbol-data/local?name=AAPL")
    ...
    def test_get_symbol_data_local_with_dates(self, client, sample_market_data):
        ...
        with patch('api.v1.market.stock.market_data_service.get_market_data', mock_get_data):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                ...
                response = client.get("/market/symbol-data/local?name=AAPL&start_date=2024-01-01&end_date=2024-01-10")
    ...
    def test_get_symbol_data_local_invalid_start_date(self, client):
        ...
        response = client.get("/market/symbol-data/local?name=AAPL&start_date=invalid-date")
    ...
    def test_get_symbol_data_local_invalid_end_date(self, client):
        ...
        # Make request with invalid date
        response = client.get("/market/symbol-data/local?name=AAPL&end_date=invalid-date")
    ...
    def test_get_symbol_data_local_no_data(self, client):
        ...
        # Setup mocks
        with patch('api.v1.market.stock.market_data_service.get_market_data', return_value=None):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbol-data/local?name=INVALID")
    ...
    def test_get_symbol_data_fresh_local_data(self, client, sample_market_data, sample_quality_info):
        """Test intelligent data selection with fresh local data."""
        # Setup mocks
        with patch('api.v1.market.stock.market_data_service.get_data_quality_info', return_value=sample_quality_info):
            with patch('api.v1.market.stock.market_data_service.get_market_data', return_value=sample_market_data):
                with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                    # Make request
                    ...
                    data = response.json()
                    assert data["symbol"] == "AAPL"
                    assert data["source"] == "local_database_fresh"
                    assert data["data_points"] == 10
    ...
    def test_get_symbol_data_fetch_fresh_data(self, client, sample_market_data):
        ...
        # Setup mocks
        with patch('api.v1.market.stock.market_data_service.get_data_quality_info', return_value={"is_fresh": False, "data_age_days": 2.0}):
            with patch('api.v1.market.stock.market_data_service.fetch_ticker_data', return_value=sample_market_data):
                with patch('api.v1.market.stock.market_data_service.store_market_data'):
                    with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                        # Mock the database session import that happens inside the function
                        with patch('app.core.database.connection.get_db_session'):
                            # Make request
                            response = client.get("/market/symbol-data?name=AAPL")
            ...
            with patch('api.v1.market.stock.market_data_service.fetch_ticker_data', return_value=None):
                with patch('api.v1.market.stock.market_data_service.get_market_data', return_value=sample_market_data):
                    with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                        ...
                        assert "stale local data" in data["warning"]
    ...
    def test_get_symbol_data_exception(self, client):
        ...
        # Setup mocks
        with patch('api.v1.market.stock.market_data_service.get_data_quality_info', side_effect=Exception("Service Error")):
            with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
                # Make request
                response = client.get("/market/symbol-data?name=AAPL")
            ...
            with patch('api.v1.market.stock.get_client_ip', return_value="127.0.0.1"):
                with patch('api.v1.market.stock.is_rate_limited', return_value=False):
                    ...
                    endpoints_and_limits = [
                        ...
                        ("/market/symbol-data/local?name=AAPL", "local_data", 20, 3600),
                        ("/market/symbol-data?name=AAPL", "intelligent_data", 15, 3600)
                    ]
                    
                    for endpoint, rate_key, max_attempts, window in endpoints_and_limits:
                        # Make request
    ...
    def test_date_parsing_edge_cases(self, client):
        ...
        for invalid_date in invalid_dates:
            response = client.get(f"/market/symbol-data/local?name=AAPL&start_date={invalid_date}")
            assert response.status_code == 400
            assert "Invalid start_date format" in response.json()["detail"]
    ...
    def test_authenticated_user_no_rate_limiting(self, client):
        ...
        with patch('api.v1.market.stock.get_optional_current_user', return_value={"id": 1, "username": "testuser"}):
            ...
            endpoints = [
                ...
                "/market/symbol-data/local?name=AAPL",
            ...
            for response_data, description in failure_scenarios:
                with patch('api.v1.market.stock.search', return_value=response_data):
                    ...
                    if response_data is None:
                        assert response.status_code == 500
                    elif "quotes" not in response_data or not isinstance(response_data.get("quotes"), list):
