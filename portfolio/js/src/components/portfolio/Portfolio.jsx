    ...
    const loadPortfolios = async () => {
        try {
            ...
            if (error.response?.status === 401) {
                ...
                toast.error('Portfolio service not found. Please check if the backend is running.');
            } else if (error.response?.status >= 500) {
                toast.error('Server error. Please try again later.');
            } else if (error.code === 'NETWORK_ERROR' || error.message.includes('Network Error')) {
                toast.error('Network error. Please check if the backend server is running on http://localhost:8000');
            } else {
    ...
    const handleCreatePortfolio = async (portfolioData) => {
        try {
            ...
            if (error.response?.status === 400) {
                ...
                toast.error(`Failed to create portfolio: ${error.response?.data?.detail || error.message}`);
            }
        }
    };
    ...
    const handleUpdatePortfolio = async (portfolioId, portfolioData) => {
        try {
            ...
            if (error.response?.status === 400) {
                ...
                toast.error(`Failed to update portfolio: ${error.response?.data?.detail || error.message}`);
            }
        }
    };
    ...
    const handleDeletePortfolio = async (portfolioId) => {
        try {
            ...
            if (error.response?.status === 401) {
                ...
                toast.error(`Failed to delete portfolio: ${error.response?.data?.detail || error.message}`);
            }
        }
    };
